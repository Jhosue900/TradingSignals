...
import WelcomeModal from '../components/Omega/Welcomemodal';
...
export default function Omega() {
  ...
  // El modal se muestra si la URL trae ?session_id=... (redirect post-pago de Stripe)
  ...
  // Cierra el modal y limpia session_id de la URL sin recargar la página
  const closeWelcome = () => {
  ...
  return (
    <>
      <div
        className={`p-6 max-w-7xl mx-auto space-y-6 transition-[filter] duration-300 ${
          showWelcome ? 'blur-sm pointer-events-none select-none' : ''
        }`}
      >
        {/* Header interno */}
      ...
      {showWelcome && (
        <WelcomeModal name={profile?.nombre} plan={profile?.plan} onClose={closeWelcome} />
      )}
    </>
  );
}